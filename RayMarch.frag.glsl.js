...
#define GetSphere(Index,Pos,Radius,Rgb)	\
{					\
	Pos = SphereData[Index*SPHERE_STRIDE+0].xyz;	\
	Radius = SphereData[Index*SPHERE_STRIDE+0].w;	\
	Rgb = SphereData[Index*SPHERE_STRIDE+1].xyz;	\
}

#define GetSpherePosRad(Index)	SphereData[Index*SPHERE_STRIDE+0].xyzw;
...
varying mat4 ScreenToWorldTransform;
vec3 ScreenToWorld(vec2 uv,float z)
{
	float x = mix( -1.0, 1.0, uv.x );
	float y = mix( 1.0, -1.0, uv.y );
	vec4 ScreenPos4 = vec4( x, y, z, 1.0 );
	//	gr: no noticable speedup
	//mat4 ScreenToWorldTransform = CameraToWorldTransform * ScreenToCameraTransform;
	//vec4 WorldPos4 = ScreenToWorldTransform * ScreenPos4;
...
{
	float Distance = length( RayPosition - SpherePosition );
	Distance -= Radius;
	return Distance;
}
...
float GetSceneDistance(vec3 RayPosition)
{
	...
	{
		vec4 SpherePosRad = GetSpherePosRad( s );
		float SphereDistance = GetDistanceToSphere(RayPosition,SpherePosRad.xyz,SpherePosRad.w);
...
float RayMarchSpherePos(TRay Ray,out float StepHeat)
{
	...
	for ( int s=0;	s<MaxSteps;	s++ )
	{
		...
		float HitDistance = Result.x;
		//float HitDistance = GetSceneDistance( Position );
...
{
	float IntersectionTime = RayMarchSpherePos( Ray, StepHeat );
	...
	return vec4( Colour, IntersectionTime );
}
